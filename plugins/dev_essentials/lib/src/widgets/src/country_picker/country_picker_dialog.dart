...
class CountryPickerDialog extends StatefulWidget {
  ...
  ///Widget to build list views item inside dialog
  ...
  ///The search empty views is displayed if nothing returns from search result
...
class SingleChoiceDialogState extends State<CountryPickerDialog> {
  ...
  Widget build(BuildContext context) {
    // Check if it's a mobile device (width < 600px is typically mobile)
    final isMobile = MediaQuery.of(context).size.width < 600;
    
    return Center(
      child: Container(
        width: isMobile 
            ? MediaQuery.of(context).size.width * 0.95 // 95% width for mobile
            : MediaQuery.of(context).size.width * 0.6, // 60% width for larger screens
        constraints: BoxConstraints(
          maxWidth: isMobile 
              ? MediaQuery.of(context).size.width * 0.95 // Full width for mobile
              : 500, // Max 500px for larger screens
          maxHeight: MediaQuery.of(context).size.height * 0.7, // 70% height for both
        ),
        child: MyAlertDialog(
          title: _buildHeader(),
          ...
          _filteredCountries = _allCountries
              .where((Country country) => widget.searchFilter == null
                  ? country.name.toLowerCase().contains(value.toLowerCase()) ||
                      country.nameAr.contains(value.toLowerCase()) ||
